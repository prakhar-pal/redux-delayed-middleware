  ...
  columns = [
    ...
    },
    {
      ...
      Cell: ({ original }) => {
        const { updatingIndex } = this.state;
        const { id } = original;
        return updatingIndex[id] ? <span className="btn btn-danger w-100"></span> : "x";
      }
    }
  ];
  ...
  render() {
    ...
    return (
      <div className="container h-75">
        <button onClick={this.updateAll} className="btn btn-primary mx-0 my-3"> Update All</button>
        <ReactTable
...
const appActions = dispatch => bindActionCreators({
  getPeople,
  updatePeople
}, dispatch);
